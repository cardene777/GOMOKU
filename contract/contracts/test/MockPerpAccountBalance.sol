...
contract MockPerpAccountBalance {
    ...
    mapping (address => mapping (address => uint256)) private _takerPositionSizes;
    ...
    function setTakerPositionSize(address account, address market, uint256 size) external {
        _takerPositionSizes[account][market] = size;
    }

    function getTakerPositionSize(address account, address market) external view returns (uint256) {
        return _takerPositionSizes[account][market];
    }
