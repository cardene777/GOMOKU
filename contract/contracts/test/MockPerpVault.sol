...
contract MockPerpVault {
    ...
    mapping (address => int) private _settlementTokenValues;
    ...
    function setSettlementTokenValue(address account, int256 tokenValue) external {
        _settlementTokenValues[account] = tokenValue;
    }

    function getSettlementTokenValue(address account) external view returns (int256) {
        return _settlementTokenValues[account];
    }
}
