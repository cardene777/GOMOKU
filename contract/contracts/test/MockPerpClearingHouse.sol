...
contract MockPerpClearingHouse {
    ...
    function openPosition(IClearingHouse.OpenPositionParams memory params) external returns (uint256, uint256) {
        (uint256 base, uint256 quote,) = openPositionFor(msg.sender, params);
        return (base, quote);
    }

    function openPositionFor(address delegate, IClearingHouse.OpenPositionParams memory params)
    public
    returns (uint256, uint256, uint256) {
        ...
        return (params.amount, params.amount * multiplier, params.amount / 100);
    }
