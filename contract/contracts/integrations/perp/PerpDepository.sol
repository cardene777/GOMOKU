    ...
    PerpDepositoryStorage
{
    ...
    /// @param _controller GMK Controller. Used for access control
    function initialize(
        address _vault,
        address _clearingHouse,
        address _marketRegistry,
        address _delegateApproval,
        ...
        contractAddresses[3] = _delegateApproval;
        contractAddresses[4] = _futuresMarket;
        contractAddresses[5] = _assetToken;
        contractAddresses[6] = _quoteToken;
        contractAddresses[7] = _controller;
        _checkContractAddresses(contractAddresses);

        vault = IVault(_vault);
        clearingHouse = IClearingHouse(_clearingHouse);
        marketRegistry = IMarketRegistry(_marketRegistry);
        delegateApproval = IDelegateApproval(_delegateApproval);
        market = _futuresMarket;
        assetToken = _assetToken;
        quoteToken = _quoteToken;
        controller = _controller;

        /// approve to use _openPositionFor
        /// action 1 = OPEN_POSITION
        delegateApproval.approve(address(this), 1);
    }
        ...
        onlyOwner
    {
        ...
        IERC20Upgradeable(insuranceToken()).safeTransferFrom(from, address(this), amount);
        ...
        onlyOwner
    {
        ...
        IERC20Upgradeable(insuranceToken()).safeTransfer(to, amount);
    ...
    /// @dev This should only happen when redeeming GMK for collateral.
    ...
    function _withdrawAsset(uint256 amount, address to) private {
        ...
        IERC20Upgradeable(assetToken).safeTransfer(to, amount);
    }
    ...
    function _placePerpOrder(
        ...
        (uint256 baseAmount, uint256 quoteAmount, uint256 feeAmount) = clearingHouse.openPositionFor(
            address(this),
            params
        );
        totalFeesPaid += feeAmount;

        emit PositionOpened(isShort, amount, amountIsInput, feeAmount, sqrtPriceLimit);
        return (baseAmount, quoteAmount);
    }
        ...
        returns (uint256)
    {
        ...
        IERC20Upgradeable(quoteToken).safeTransfer(address(controller), quoteAmount);
        return quoteAmount;
    }
    ...
    function getUnrealizedPnl() public view returns (int256) {
        return redeemableUnderManagement.toInt256() - getPositionValue().toInt256();
    }
    ...
    /// @param minAmountOut The minimum amount of base token to receive in the swap to proceed with rebalancing.
    /// @param polarity the direction of the rebalance. -1 to rebalance negative PnL, +1 for positive PnL
    /// @param sqrtPriceLimitX96 The target price when performing the swap on the spot DEX.
    ...
    function rebalance(
        ...
        if (polarity == -1) {
            ...
            // return _rebalancePositivePnlWithSwap(amount, minAmountOut, sqrtPriceLimitX96, swapPoolFee);
        } else {
    ...
    // swap base => quote
    // Transfer shortfall from swap from `msg.sender`
    ...
    function _rebalanceNegativePnlWithSwap(
        ...
        int256 shortFall = quoteAmount.fromDecimalToDecimal(18, ERC20(quoteToken).decimals()).toInt256() - quoteAmountOut.toInt256();
        if (shortFall > 0) {
    ...
    //         poolFee: swapPoolFee
    //     });
    //     IERC20(quoteToken).approve(address(spotSwapper), normalizedAmount);
    //     uint256 baseAmountFromSwap = spotSwapper.swapExactInput(params);
    //     // can we wait to deposit this after we know the shortfall?
    //     netAssetDeposits += baseAmountFromSwap;
    //     IERC20(assetToken).approve(address(vault), baseAmountFromSwap);
    ...
    //         0
    //     );
    //     int256 shortFall = baseAmount.toInt256() - baseAmountFromSwap.toInt256();
    //     if (shortFall > 0) {
    //         IERC20Upgradeable(assetToken).safeTransferFrom(
    //             msg.sender,
    //             address(this),
    //             shortFall.toUint256()
    //         );
    //         netAssetDeposits += shortFall;
    //         IERC20(assetToken).approve(address(vault), shortFall.toUint256());
    //         vault.deposit(assetToken, shortFall.toUint256());
    //     } else if (shortFall < 0) {
    //         // we got excess tokens from swap. Send them to account paying for rebalance
    //         IERC20Upgradeable(assetToken).safeTransfer(
    //             msg.sender,
    //             _abs(shortFall)
    //         );
    //     }
    ...
    /// For negative PnL, `msg.sender` provides amount in quote token.
    /// For this call to succeed the `msg.sender` must have approved this contract to spend amount (for negative PnL),
    ...
    /// @param minAmountOut The minimum amount of base token the caller receives. Transaction will
    ...
    function rebalanceLite(
        ...
        if (polarity == -1) {
            return
                _rebalanceNegativePnlLite(amount, minAmountOut);
        } else if (polarity == 1) {
            // disable rebalancing positive PnL
            revert PositivePnlRebalanceDisabled(msg.sender);
            // return _rebalancePositivePnlLite(amount, minAmountOut);
        } else {
            revert InvalidRebalance(polarity);
        }
    }
    ...
    function _rebalanceNegativePnlLite(
        ...
        IERC20Upgradeable(quoteToken).safeTransferFrom(msg.sender, address(this), amount);
        IERC20Upgradeable(quoteToken).approve(address(vault), amount);
        ...
        if (baseAmount < minAmountOut) {
            revert MinAmountNotMet(minAmountOut, baseAmount);
        }

        vault.withdraw(assetToken, baseAmount);
        netAssetDeposits -= baseAmount;
        IERC20Upgradeable(assetToken).safeTransfer(msg.sender, baseAmount);
    ...
    //         baseIncludingFee
    //     );
    //     IERC20Upgradeable(assetToken).approve(address(vault), baseIncludingFee);
    //     netAssetDeposits += baseIncludingFee;
    //     vault.deposit(assetToken, baseIncludingFee);
    //     vault.withdraw(quoteToken, amount);
    //     IERC20Upgradeable(quoteToken).safeTransfer(msg.sender, amount);
    ...
    function supportedAssets() external override view returns (address[] memory) {
        ...
        (assetList[0], assetList[1]) = (assetToken, quoteToken);
        return assetList;
    }
    ...
    function getPositionValue() public view returns (uint256) {
        uint256 markPrice = getMarkPriceTwap(900);
    ...
    function getDebtValue(address account) external view returns (uint256) {
        int256 settlementTokenValue = vault.getSettlementTokenValue(account);
        uint256 settlementTokenDebt = settlementTokenValue < 0 ? _abs(settlementTokenValue) : 0;
        return settlementTokenDebt;
    }
    ...
    function _abs(int256 value) private pure returns (uint256) {
        return value >= 0 ? value.toUint256() : (-1 * value).toUint256();
    }
