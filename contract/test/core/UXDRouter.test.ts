...
import { TestDepository } from "../../types";

chai.use(solidity);

describe("GMKRouter", async () => {
  let router: Contract;
  ...
  it("can register depositories", async () => {
    ...
    await (
      await router.registerDepository(depMarket1Coll1.address, asset1)
    ).wait();
    await (
      await router.registerDepository(depMarket1Coll2.address, asset1)
    ).wait();
    await (
      await router.registerDepository(depMarket2Coll1.address, asset2)
    ).wait();
    await (
      await router.registerDepository(depMarket2Coll2.address, asset2)
    ).wait();
  ...
  it("can unregister depository", async () => {
    ...
    await (
      await router.unregisterDepository(depository.address, asset1)
    ).wait();
  ...
  it("fails if registering duplicate depository", async () => {
    await (await router.registerDepository(depository.address, asset1)).wait();
    await expect(router.registerDepository(depository.address, asset1)).to.be
      .reverted;
  });

  it("fails if unregistering unregistered depository", async () => {
    await expect(router.unregisterDepository(depository.address, asset1)).to.be
      .reverted;
