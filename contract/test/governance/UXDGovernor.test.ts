...
describe("UXDGovernor", async () => {
  ...
  it("reverts if approve fails", async () => {
    ...
    const callData = governor.interface.encodeFunctionData("approveERC20", [
      token.address,
    ...
    await (
      await governor.queue([governor.address], [0], [callData], descriptionHash)
    ).wait();
    await expect(
      governor.execute([governor.address], [0], [callData], descriptionHash)
    ).to.be.reverted;
  });
  ...
  it("can transfer erc20 tokens", async () => {
    ...
    const proposalDescription = "approve erc20";
    const descriptionHash = ethers.utils.id(proposalDescription);
    const callData = governor.interface.encodeFunctionData("transferERC20", [
      token.address,
      bob.address,
      amount,
    ]);
