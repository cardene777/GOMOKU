...
describe("UXDTimelockController", async () => {
  ...
  it("reverts if approve fails", async () => {
    ...
    const callData = timelock.interface.encodeFunctionData("approveERC20", [
      token.address,
    ...
    await (
      await governor.queue([timelock.address], [0], [callData], descriptionHash)
    ).wait();
    await expect(
      governor.execute([timelock.address], [0], [callData], descriptionHash)
    ).to.be.reverted;
  });
  ...
  it("can transfer erc20 tokens", async () => {
    ...
    const proposalDescription = "approve erc20";
    const descriptionHash = ethers.utils.id(proposalDescription);
    const callData = timelock.interface.encodeFunctionData("transferERC20", [
      token.address,
      bob.address,
      amount,
    ]);
  ...
  it("can transfer eth", async () => {
    ...
    const descriptionHash = ethers.utils.id(proposalDescription);
    const callData = timelock.interface.encodeFunctionData("transferETH", [
      bob.address,
      amount,
    ]);
