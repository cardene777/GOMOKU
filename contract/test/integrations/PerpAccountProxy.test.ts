...
describe("PerpAccountProxy", async () => {
  ...
  beforeEach(async () => {
    ...
    perpVault = (await (
      await ethers.getContractFactory("MockPerpVault")
  ...
  it("reads state from external contracts", async () => {
    ...
    await (
      await perpAccountBalance.setOpenNotional(
        account,
        market,
        defaults.openNotional
      )
    ).wait();
    await (
      await perpAccountBalance.setPnlAndPendingFee(
        account,
        defaults.pnlAndPendingFee
      )
    ).wait();
    await (
      await perpAccountBalance.setTotalDebtValue(account, defaults.debtValue)
    ).wait();
    await (
      await perpAccountBalance.setTotalPositionSize(defaults.totalPositionSize)
    ).wait();
    await (
      await perpAccountBalance.setTotalPositionValue(
        defaults.totalPositionValue
      )
    ).wait();
    await (
      await perpAccountBalance.setTotalAbsPositionValue(
        account,
        defaults.absPositionValue
      )
    ).wait();

    await (
      await perpVault.setFreeCollateral(account, defaults.freeCollateral)
    ).wait();
    await (
      await perpVault.setBalanceByToken(account, token, defaults.balanceByToken)
    ).wait();
    await (
      await perpVault.setFreeCollateralByToken(
        account,
        token,
        defaults.freeCollateralByToken
      )
    ).wait();
    ...
    const totalPositionValue = await proxy.getTotalPositionValue(
      account,
      market
    );
    const absPositionValue = await proxy.getTotalAbsPositionValue(account);
    const debtValue = await proxy.getTotalDebtValue(account);
    const freeCollateralByToken = await proxy.getFreeCollateralByToken(
      account,
      token
    );
