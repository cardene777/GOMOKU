...
import { EthereumConfig } from "../../config/configs";
...
async function main() {
  ...
  checkNetwork(hre, [NetworkName.Goerli, NetworkName.Ethereum]);

  config = (await loadConfig(hre)) as EthereumConfig;
...
async function setup() {
  ...
  const mintCap = ethers.utils.parseEther("1000000");
