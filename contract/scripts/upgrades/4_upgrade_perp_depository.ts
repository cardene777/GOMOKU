...
async function upgradeContracts() {
  ...
  depository = Depository.attach(
    depositories.perpetualProtocol!
  ) as PerpDepository;
  await (await depository.upgradeTo(depositoryImpl.depositoryImpl)).wait();
  console.log(
    `PerpDepository upgraded [Data => Logic] [${depository.address} => ${depositoryImpl.depositoryImpl} ✅`
  );
}
...
async function verifyContracts() {
  ...
  await hre
    ...
    .catch((err) =>
      console.log("caught verification error[depositoryImpl]", err)
    );
