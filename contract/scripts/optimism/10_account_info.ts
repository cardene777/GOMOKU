...
import { PerpAccountProxy, PerpDepository } from "../../types";
import {
  loadConfig,
  loadCoreContracts,
  loadDepositories,
...
async function main() {
  ...
  const proxy: PerpAccountProxy = AccountProxy.attach(
    periphery.accountProxy
  ) as PerpAccountProxy;
  ...
  const openNotional = await proxy.getTotalOpenNotional(
    address,
    config.contracts.PerpVETHMarket
  );
  ...
  console.log(
    "current debt value = ",
    ethers.utils.formatEther(currentDebtValue)
  );

  const insuranceDeposited = await depository.insuranceDeposited();
  console.log(
    "Insurance deposited = ",
    ethers.utils.formatUnits(insuranceDeposited, 6)
  );

  let perpPnl = await proxy.getPnlAndPendingFee(address);
  console.log(
    `PERP PnL = {realized = ${ethers.utils.formatEther(
      perpPnl[0]
    )}, unrealized = ${ethers.utils.formatEther(
      perpPnl[1]
    )}, pendingFee = ${ethers.utils.formatEther(perpPnl[2])}}`
  );

  let markPriceTwap = await depository.getMarkPriceTwap("0");
  console.log(
    `MarkPriceTwap = ${markPriceTwap} <> ${ethers.utils.formatEther(
      markPriceTwap
    )}`
  );
  const positionSize = await proxy.getTotalPositionSize(
    address,
    config.contracts.PerpVETHMarket
  );
  ...
  console.log(
    "Abs Position value = ",
    ethers.utils.formatEther(absPositionValue)
  );
  ...
  console.log(
    "Free collateral = ",
    ethers.utils.formatUnits(freeCollateral, 6)
  );

  const freeUsdcCollateral = await proxy.getFreeCollateralByToken(
    address,
    config.tokens.USDC
  );
  console.log(
    "Free USDC collateral = ",
    ethers.utils.formatUnits(freeUsdcCollateral, "6")
  );

  const freeEthCollateral = await proxy.getFreeCollateralByToken(
    address,
    config.tokens.WETH
  );
  console.log(
    "Free ETH collateral = ",
    ethers.utils.formatEther(freeEthCollateral)
  );

  const totalFeesPaid = await depository.totalFeesPaid();
  console.log("totalFeesPaid = ", ethers.utils.formatEther(totalFeesPaid));
  ...
  console.log(
    `Total WETH balance: ", ${ethers.utils.formatEther(
      ethBalace
    )}: [${ethMarkValue} | ${ethIndexValue}]`
  );

  const usdcBalance = await proxy.getBalanceByToken(
    address,
    config.tokens.USDC
  );
  console.log(
...
async function getPerpPrices(tickerSymbol: string) {
  const prices = await perp.markets.getMarket({ tickerSymbol }).getPrices();
  return prices;
}
