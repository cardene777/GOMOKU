...
async function main() {
  ...
  const depository = PerpDepository.attach(
  ...
  console.log(
    "Perp position value before = ",
    ethers.utils.formatEther(positionValue)
  );
  let positionSize = await proxy.getTotalPositionSize(
    depositories.perpetualProtocol,
    config.contracts.PerpVETHMarket
  );
  console.log(
    "Perp position size before = ",
    ethers.utils.formatEther(positionSize)
  );
  ...
  const tx = await depository.rebalanceLite(usdcAmount, polarity, targetPrice);
  console.log("rebalance tx = ", tx.hash);
  positionValue = await depository.getPositionValue();
  console.log(
    "Perp position value after = ",
    ethers.utils.formatEther(positionValue)
  );
  positionSize = await proxy.getTotalPositionSize(
    depositories.perpetualProtocol,
    config.contracts.PerpVETHMarket
  );
  console.log(
    "Perp position size after = ",
    ethers.utils.formatEther(positionSize)
  );
