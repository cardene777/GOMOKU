...
async function deployContracts() {
  ...
  tokenReceiver = config.addresses.TokenReceiver;
...
async function verifyContracts() {
  ...
  await verify(councilToken.address, [tokenReceiver]).catch((err) =>
    console.log("caught verification error", err)
  );

  // Timelock
  console.log(`> Verifying timelock: ${timelock.address}`);
  await verify(timelock.address, [minDelay, proposers, executors]).catch(
    (err) => console.log("caught verification error", err)
  );
