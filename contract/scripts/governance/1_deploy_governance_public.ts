...
import { UXDGovernor, UXDTimelockController, UXPToken } from "../../types";
...
async function deployContracts() {
  ...
  tokenCustodian = config.addresses.MultisigSafe;
  ...
  uxpToken = (await UXPToken.deploy(
    tokenCustodian,
    tokenTotalSupply,
    lzEndpoint
  )) as UXPToken;
...
async function verifyContracts() {
  ...
  verify(timelock.address, [minDelay, proposers, executors]).catch((err) =>
    console.log("caught verification error", err)
  );

  // Governor
  console.log(`> Verifying governor: ${governor.address}`);
  verify(governor.address, [uxpToken.address, timelock.address]).catch((err) =>
    console.log("caught verification error", err)
  );
